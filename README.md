...
風向は`byte[24]`下位4ビット（`0x00`=停止、`0x0F`=スイング）として確認済みで、
`swing on/off`で操作できる。
風量は`byte[24]`上位4ビットで、今回の実機確認では`0xA`=自動、`0xB`=静音、
`0x3`〜`0x7`=風量1〜5として変化した。`fan auto|quiet|1..5`で操作できる。
入タイマーを1回押した状態は`byte[21]`の入タイマービットがONになり、
`byte[26]〜[28]`が120分（2:00）を示した。`timer-on 120`で生成できる。
その状態で切タイマーを1回押すと切タイマービットもONになり、切タイマーは
60分（1:00）になった。`timer-off 60`で生成できる。
取消を押すと両タイマーのビットがOFFになり、時刻フィールドも無効値へ戻った。
`timer-cancel`で同じ状態を生成できる。
